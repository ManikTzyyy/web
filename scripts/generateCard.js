document.addEventListener("DOMContentLoaded", () => {
  const swiperContainer = document.getElementById("recent-project-wrapper");

  fetch("scripts/data.json")
    .then((res) => res.json())
    .then((projects) => {
      // Ambil hanya 3 project terbaru
      ...
      recentProjects.forEach((project) => {
        const cardHTML = `
          ...
          `;
        swiperContainer.innerHTML += cardHTML;
      });
    })
    .catch((err) => console.error("Gagal load recent projects", err));
});
